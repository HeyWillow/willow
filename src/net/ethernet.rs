...
pub(crate) fn initialize() -> Result<NonNull<esp_netif_t>, EspError> {
    ...
    ui::show_connecting("Connecting to Ethernet ...");
    ...
    let network_interface =
        NonNull::new(unsafe { esp_netif_new(&raw const network_interface_configuration) })
            .ok_or_else(|| {
    ...
    let mut address = [0_u8; 6];
    check(
        unsafe { esp_read_mac(address.as_mut_ptr(), esp_mac_type_t_ESP_MAC_ETH) },
        "failed to derive Ethernet MAC address",
    )?;
    check(
        unsafe {
            esp_eth_ioctl(
    ...
    check(
        unsafe { esp_netif_attach(network_interface.as_ptr(), glue.cast()) },
        "failed to attach Ethernet network interface",
    )?;

    let _ = set_hostname(network_interface, esp_mac_type_t_ESP_MAC_ETH);
    ...
    while !unsafe { esp_netif_is_netif_up(network_interface.as_ptr()) } {
