...
//!
//! C publishes semantic UI changes through the temporary functions at the
//! bottom of this module. A dedicated Rust task snapshots that state, renders
//! a complete RGB565 frame in PSRAM, and flushes it through a small internal
//! DMA buffer. Oversized lines are cached in PSRAM and only their viewport is
//! flushed while scrolling. Touch polling is also Rust-owned and sends cancel
//! events to the existing Rust-owned recorder queue.
...
use willow_schema::config::v1::SpeechRecognitionMode;
...
impl UiState {
    fn new() -> Self {
        ...
        let startup = match crate::config::config().and_then(|config| config.speech_rec_mode) {
            Some(SpeechRecognitionMode::Multinet) if cfg!(willow_support_multinet) => {
                "Starting up (local)..."
            }
            Some(SpeechRecognitionMode::Multinet) => "Multinet Not Supported",
            _ => "Starting up (server)...",
        };
        state.set_line(2, startup, LineAlignment::Center, Rgb565::WHITE);
        state
    }
...
#[derive(Clone)]
struct TonnelierTextStyle {
    background: Rgb565,
    color: Rgb565,
    font: Font<'static>,
}

impl TonnelierTextStyle {
    fn new(font: Font<'static>, color: Rgb565, background: Rgb565) -> Self {
        Self {
            background,
            color,
            font,
    ...
    fn blend(&self, coverage: f32) -> Rgb565 {
        let foreground: Rgb888 = self.color.into();
        let background: Rgb888 = self.background.into();
        let alpha = (coverage * 255.0) as u16;
        let inverse = 255 - alpha;
        Rgb888::new(
            ((alpha * u16::from(foreground.r()) + inverse * u16::from(background.r())) / 255) as u8,
            ((alpha * u16::from(foreground.g()) + inverse * u16::from(background.g())) / 255) as u8,
            ((alpha * u16::from(foreground.b()) + inverse * u16::from(background.b())) / 255) as u8,
        )
        .into()
    }

    fn layout<'a>(&'a self, text: &'a str) -> Vec<rusttype::PositionedGlyph<'a>> {
        let scale = Scale::uniform(FONT_SIZE as f32);
        let ascent = self.font.v_metrics(scale).ascent;
        self.font.layout(text, scale, point(0.0, ascent)).collect()
    }

    fn width(glyphs: &[rusttype::PositionedGlyph<'_>]) -> u32 {
        let advance = glyphs
            .last()
        ...
        let pixels = glyphs
            .iter()
            .filter_map(rusttype::PositionedGlyph::pixel_bounding_box)
            .map(|bounds| bounds.max.x)
            .max()
            .unwrap_or(0);
        advance.max(pixels).max(0) as u32
    }
}

impl TextRenderer for TonnelierTextStyle {
    type Color = Rgb565;
        ...
        D: DrawTarget<Color = Self::Color>,
    {
        ...
        for glyph in glyphs {
            ...
            glyph.draw(|x, y, coverage| {
                pixels.push(Pixel(
                    position + Point::new(x as i32 + bounds.min.x, y as i32 + bounds.min.y),
                    self.blend(coverage),
                ));
            });
        }
    ...
    fn measure_string(&self, text: &str, position: Point, _baseline: Baseline) -> TextMetrics {
        let width = Self::width(&self.layout(text));
        TextMetrics {
            bounding_box: Rectangle::new(position, Size::new(width, FONT_SIZE)),
...
impl Renderer {
    fn new() -> Result<Self, EspError> {
        ...
        Ok(Self {
            ...
            font: Font::try_from_bytes(
                include_bytes!("../assets/fonts/Tonnelier-Regular.ttf") as &[u8]
            )
            .ok_or_else(|| {
    ...
    fn render(&mut self, state: &UiState) -> Result<(), EspError> {
        self.framebuffer.fill(willow_background());
        self.scrolls = std::array::from_fn(|_| None);
        ...
        let _ = Text::with_text_style(
            ...
            self.font_style(Rgb565::WHITE, willow_background()),
            centered,
        )
        ...
        for (index, line) in state.lines.iter().enumerate() {
            ...
            let font_style = self.font_style(line.color, willow_background());
            let text_width = font_style
                .measure_string(&line.text, Point::zero(), Baseline::Top)
                .bounding_box
                .size
                .width as usize;
            if line.scroll && text_width > SCROLL_VIEWPORT_WIDTH {
                let mut bitmap = Framebuffer::with_size(text_width, SCROLL_VIEWPORT_HEIGHT)?;
                ...
                self.scrolls[index] = Some(ScrollAnimation {
                    bitmap,
                    max_offset: text_width - SCROLL_VIEWPORT_WIDTH,
                    offset: 0,
                    started: Instant::now(),
                });
                continue;
            ...
            let alignment = match line.alignment {
                LineAlignment::Left => Alignment::Left,
        ...
        if !matches!(state.cancel_action, CancelAction::None) {
            ...
            let _ = Text::with_text_style(
                ...
                self.font_style(Rgb565::BLACK, willow_accent()),
                centered,
            )
            .draw(&mut self.framebuffer);
        }
    ...
    fn font_style(&self, color: Rgb565, background: Rgb565) -> TonnelierTextStyle {
        TonnelierTextStyle::new(self.font.clone(), color, background)
    }
...
fn cancel(state: &Arc<Mutex<UiState>>, stop_event: i32) {
    ...
    match action {
        ...
        CancelAction::Recording => {
            if let Err(error) = crate::audio::send_recorder_event(stop_event) {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rust_ui_show_connecting(message: *const c_char) {
    let Some(message) = (unsafe { text(message) }) else {
        return;
    };
...
pub unsafe extern "C" fn rust_ui_show_error(primary: *const c_char, secondary: *const c_char) {
    let primary = unsafe { text(primary) };
    let secondary = unsafe { text(secondary) };
    update(|state| {
        state.hide(&[0, 1, 4]);
        state.lines[2].visible = false;
...
#[unsafe(no_mangle)]
pub extern "C" fn rust_ui_show_unrecognized() {
    update(|state| {
        state.set_line(3, "Unrecognized Command", LineAlignment::Left, Rgb565::RED);
    });
}

#[unsafe(no_mangle)]
pub extern "C" fn rust_ui_show_listening(local: bool) {
    update(|state| {
        state.hide(&[0, 1, 3, 4]);
...
pub unsafe extern "C" fn rust_ui_show_notification(
    ...
    let message = unsafe { text(message) }.unwrap_or_else(|| "Notification Active".into());
    update(|state| {
        state.hide(&[0, 1, 3, 4]);
